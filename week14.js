...
const myInformation = ['Екатерина', '14.02.1982', 'февраль', 'Водолей'];
...
let lastIndex = countries.length;
console.log(countries[lastIndex - 1]);
...
let changedArray = [...countries];
console.log(changedArray.shift());
...
let greatCities = ['Санкт-Петербург', 'Москва', 'Екатеринбург'];
russianCities = [...greatCities, 'Казань', 'Нижний Новгород'];
...
function sum() {
    let ind = numbers.length;
    return numbers[0] + numbers[ind - 1];
};
console.log(sum());
...
let sortAndPrint = [...randomNumbers].sort();
